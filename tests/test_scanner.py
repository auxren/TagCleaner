...
class TestInfoFileFilter:
    """Tool-output files masquerading as info.txt (aucdtect, foo_dr,
    artwork listings, tracker spam) used to outrank the real info.txt by
    byte size and feed the parser garbage. Filter them by filename; if
    that isn't enough, sniff the body."""

    @pytest.mark.parametrize("junk_name", [
        "CD1.aucdtect.txt",
        "folder.aucdtect.txt",
        "aucdtect.txt",
        "BS 1971-09-25 Long Beach - foo_dr.txt",
        "Artwork.txt",
        "Torrent downloaded from Demonoid.com.txt",
        "fpt.txt",
    ])
    def test_junk_filename_filtered(self, tmp_path: Path, make_flac, junk_name):
        folder = tmp_path / "show"
        folder.mkdir()
        make_flac(folder / "01 a.flac")
        (folder / "info.txt").write_text("Artist\n1999-01-01\n", encoding="utf-8")
        # Junk file is *larger* than the real info — without a filter it wins.
        (folder / junk_name).write_text("x" * 10_000, encoding="utf-8")
        result = _classify(folder)
        assert result is not None
        _, info, _ = result
        picked = [p.name for p, _ in info]
        assert junk_name not in picked
        assert "info.txt" in picked

    def test_checksum_body_in_disc_named_file_rejected(
        self, tmp_path: Path, make_flac
    ):
        # ACDC/Playhouse 1986 ships `Disc 1/Disc 1.txt` whose body is
        # MD5summer output. Filename passes the filter; content sniff
        # must catch it.
        folder = tmp_path / "Disc 1"
        folder.mkdir()
        make_flac(folder / "Track 01.flac")
        (folder / "Disc 1.txt").write_text(
            "# MD5 checksums generated by MD5summer\n"
            "ae118f82eb8824f579ec7c0a5b4f4397 *Track 01.flac\n"
            "b17da79b07dd90c7e2fa8f8a44342c06 *Track 02.flac\n"
            "80a50c43f36f7bf4e91033c92cdddd4f *Track 03.flac\n"
            "4d149396b759d477802ec15186c15cb7 *Track 04.flac\n",
            encoding="utf-8",
        )
        result = _classify(folder)
        assert result is not None
        _, info, _ = result
        assert info == [], f"expected no info candidate, got {info!r}"

    def test_ffp_style_body_rejected(self, tmp_path: Path, make_flac):
        folder = tmp_path / "show"
        folder.mkdir()
        make_flac(folder / "01 a.flac")
        # File named innocuously but body is FFP (FLAC fingerprint) lines.
        (folder / "notes.txt").write_text(
            "01.flac:e39973369f9d086c10a66b13ad74e8c9\n"
            "02.flac:68ff03dd9917016ecbf360d37eb04184\n"
            "03.flac:ebb4e096aafb9c38198a5b112f7cc2e2\n"
            "04.flac:d9bb9a48adb05f9e6ddecc2077b96b57\n",
            encoding="utf-8",
        )
        result = _classify(folder)
        assert result is not None
    ...
    def test_plain_prose_body_kept(self, tmp_path: Path, make_flac):
        # Sanity check: real prose info.txt must survive the content sniff.
        folder = tmp_path / "show"
        folder.mkdir()
        make_flac(folder / "01 a.flac")
        ...
        assert result is not None
        _, info, _ = result
        assert [p.name for p, _ in info] == ["info.txt"]
